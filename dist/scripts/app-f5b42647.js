...
angular.module('icons')
  .controller('iconDetailCtrl', ["$scope", "$rootScope", "$stateParams", "$http", function ($scope, $rootScope, $stateParams, $http) {
    ...
    if (angular.isDefined(iconId)) {
      $http.get('/api/icon/' + iconId).success(function(icon) {
        $scope.icon = icon;
      }).error(function() {
      	$rootScope.$broadcast('iconsDisplayMessage', {
      		type: "alert",
