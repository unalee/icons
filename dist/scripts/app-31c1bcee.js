...
angular.module('icons')
	.service('userService', ["$http", "$rootScope", "$state", "localStorageService", function($http, $rootScope, $state, localStorageService) {
		...
		userAPI.logIn = function(creds) {
			debugger;
			localStorageService.set('userToken', 'faketoken!');
...
angular.module('icons')
	.controller('uploadController', ["$scope", "$rootScope", "$http", function($scope, $rootScope, $http) {
    ...
    $scope.upload = function (file) {
      debugger;
        $http.post('/api/icon', file).success(function(res, status, headers, config) {
          // debugger;
        }).error(function(data, status, headers, config) {
          // debugger;
  				console.error(data, status, headers)
