...
angular.module('icons')
  .controller('iconDetailCtrl', ["$scope", "$rootScope", "$stateParams", "$state", "dataService", function ($scope, $rootScope, $stateParams, $state, dataService) {
    ...
    $scope.save = function() {
      var updateIconId = $scope.icon._id;
      dataService.updateIcon(updateIconId).then(function(res) {
        $rootScope.$broadcast('iconsDisplayMessage', {
          type: "success",
...
$templateCache.put("app/icon/icon.html","<div class=\"icon\" ng-controller=\"iconDetailCtrl\"><div class=\"row\"><div class=\"large-12 columns\" ng-if=\"icon.isOwnIcon\"><div class=\"icon-functions\"><button ng-if=\"!editMode\" class=\"button secondary\" ng-click=\"edit()\">Edit</button> <button ng-if=\"!editMode\" class=\"button alert\" ng-click=\"delete()\">Delete</button> <button ng-if=\"editMode\" class=\"button secondary\" ng-click=\"cancel()\">Cancel</button> <button ng-if=\"editMode\" class=\"button success\" ng-click=\"save()\">Save</button></div></div><div class=\"large-8 small-12 columns\"><h2>{{icon.title}}</h2><img class=\"graphic-item\" ng-src=\"{{icon.url}}\" title=\"{{icon.title}}\"></div><div class=\"large-4 small-12 columns\"><div class=\"detail authors\"><h4>Authors:</h4><p><a ng-repeat=\"author in icon.authors\" ng-href=\"/#/user/{{author._id}}\">{{author.name}}</a></p></div><div class=\"detail added\"><h4>Added:</h4><p>{{icon.created | moment : \"MMMM D Y\"}}</p></div><div class=\"detail location\"><h4>Location:</h4><p>{{icon.location}}</p></div><div class=\"detail story\"><h4>Story:</h4><p>{{icon.story}}</p></div><div class=\"detail tags\"><h4>Tags:</h4><p><a class=\"tag\" ng-repeat=\"tag in icon.tags\" ui-sref=\"iconList({tag:tag})\">{{tag}}{{$last ? \'\' : \', \'}}</a></p></div><div ng-if=\"icon.remixes\" class=\"detail related\"><h4>Remixes:</h4><div class=\"related-icon\" ng-repeat=\"id in icon.remixes\"><h5>{{icons[id].title}}</h5><a ui-sref=\"iconDetail({iconId:id})\"><img class=\"graphic-item\" ng-src=\"/assets/images/{{icons[id].filename}}\" title=\"{{icons[id].title}}\"></a></div></div><div ng-if=\"icon.parent\" class=\"detail related\"><h4>Created With:</h4><div class=\"related-icon\" ng-repeat=\"id in icon.parent\"><h5>{{icons[id].title}}</h5><a href=\"/#/icon?iconId={{id}}\"><img class=\"graphic-item\" ng-src=\"/assets/images/{{icons[id].filename}}\" title=\"{{icons[id].title}}\"></a></div></div></div></div></div>");
