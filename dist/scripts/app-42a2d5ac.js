...
angular.module('icons')
  .controller('iconDetailCtrl', ["$scope", "$rootScope", "$stateParams", "dataService", "$http", function ($scope, $rootScope, $stateParams, dataService, $http) {
    ...
    if (angular.isDefined(iconId)) {
      dataService.getId(iconId).then(function(icon) {
        $scope.icon = icon;
        debugger;
      }, function(error) {
        $rootScope.$broadcast('iconsDisplayMessage', {
...
angular.module('icons')
	.service('userService', ["$http", "$rootScope", "$state", "userService", function($http, $rootScope, $state, userService) {
