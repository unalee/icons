...
describe("Registration Tests", function() {
  ...
  describe('/auth/signup', function() {
    ...
    it('should not double-register a user that exists in the system',
      function(done) {
        // this.timeout(10000);
        api.post('/auth/signup').send({
          email: 'becky@made.com',
  ...
  describe('/auth/forgot', function() {
    ...
    it(
      'should reset a password token if there is a token on that account',
      function(done) {
        api.post('/auth/forgot')
          .send({
            email: 'becky@made.com'
    ...
    it.skip('should return all users on the db to an admin', function(done) {
      api.get('/api/user')
        ...
        .then(function(data) {
          ...
          expect(data.body[0]).to.include.keys('name', '_id',
            'email')
